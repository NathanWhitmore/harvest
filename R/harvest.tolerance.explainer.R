...
harvest.tolerance.explainer <- function(name, site, area,units, threshold, iterations, growth,density, biomass, offtake, seed) {
  ...
  sup <- ggplot()+
    ...
    labs(
      caption =
        "\nNotes: The distribution of each parameter have been standardised by dividing by the
median (not mean, as some distributions may be non-linear). The distributions shown
represent the intervals between the 1st and 99th percentiles. The impact of each
parameter to the overall outcome is directly proportional to its variability. To improve
your estimate you should try to improve the precision of the most variable parameter first.",
